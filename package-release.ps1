...
#
# Produces release-staging/kcd2-mempatch-<version>.zip with the explicit
...
#   - plugins/mempatch.asi         (this project's build output)
#
...
#   pwsh ./package-release.ps1 -Version 1.0.0
#
# Prereqs: build.ps1 must have already produced build/Release/mempatch.asi.
...
$Asi = Join-Path $RepoRoot "build/Release/mempatch.asi"
$Staging = Join-Path $RepoRoot "release-staging"
$Download = Join-Path $RepoRoot "_download"

if (-not (Test-Path $Asi)) {
    throw "mempatch.asi not found at $Asi — run build.ps1 first"
}
...
The rest of this distribution (mempatch.asi etc.) is MIT-licensed; see the
kcd2-mempatch repository at https://github.com/violetanvil/kcd2-mempatch for
its license.
...
# Copy mempatch.asi
Write-Host "Copying mempatch.asi..." -ForegroundColor Cyan
Copy-Item $Asi "$Staging/plugins/mempatch.asi"

# Build the zip
$ZipName = "kcd2-mempatch-$Version.zip"
