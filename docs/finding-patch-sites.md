...
You want to ship a kcd2-mempatch plugin, but you don't yet know *what
...
The bootstrap ASI kcd2-mempatch is forked from. Its `cpp/src/dllmain.cpp`
contains the verified AOB signatures for `lua_pcall` and the engine's
`update` tick function — patterns that kcd2-mempatch itself relies on.
If you need to hook into the Lua VM (or the game's tick), start there.
