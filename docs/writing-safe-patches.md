...
A short guide for mod authors using mempatch to ship a memory patch.
...
## Code injection is not supported

mempatch only handles **same-length byte rewrites**. If you need to
**add code** to the game — a new branch, a function detour, a code
cave, anything where `replacement.length != original.length` — this
tool is not the right one. The right tools are:

- **MinHook** (vendored inside mempatch but exposed only for the
  engine's own use) — provides function-level detours with trampolines.
  Write a custom ASI that uses MinHook directly.
- **Code-cave techniques** — find unused regions in the existing
  binary, write your new code there, install a 5-byte `jmp` at the
  original site. Again, custom ASI.

The reason mempatch doesn't grow into a code-injection framework is
that the architecture is different enough — named code regions,
plugin-to-plugin symbol references, trampoline allocation — to justify
a separate project. A future v2 may revisit this with an `[[inject]]`
schema alongside the current `[[patch]]` schema.
