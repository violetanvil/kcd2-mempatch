...
bool ParseOnePatch(const toml::table& t,
                   ...
                   mp::patch::PatchEntry& out,
                   std::string& err) {
    using namespace mp::patch;
...
void LoadOneFile(const fs::path& path) {
    ...
    try {
        ...
        // Top-level [mempatch] section
        if (auto* mp = doc.get("mempatch"); mp && mp->is_table()) {
            const auto& tbl = *mp->as_table();
            if (OptBool(tbl, "dry_run", false)) {
                mp::patch::g_dryRun = true;
        ...
        if (auto* arr = doc.get("patch"); arr && arr->is_array()) {
            for (const auto& elem : *arr->as_array()) {
                ...
                if (ParseOnePatch(*elem.as_table(), fileLabel, entry, err)) {
                    ...
                    mp::patch::g_patches.push_back(std::move(entry));
                } else {
...
void LoadAllConfigs(const std::wstring& pluginsDir) {
    ...
    for (const auto& entry : fs::directory_iterator(root, ec)) {
        ...
        ++folders;
        fs::path candidate = entry.path() / "mempatch.toml";
    ...
    std::sort(mp::patch::g_patches.begin(), mp::patch::g_patches.end(),
              [](const mp::patch::PatchEntry& a, const mp::patch::PatchEntry& b) {
                  if (a.priority != b.priority) return a.priority < b.priority;
                  return a.name < b.name;
              });

    log::InfoF("Discovered %zu patch(es) from %zu config file(s) across %zu plugin folder(s)",
               mp::patch::g_patches.size(), files, folders);
}
