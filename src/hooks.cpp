    ...
    if (!done.load(std::memory_order_acquire)) {
        ...
        if (L) {
            ...
            if (done.compare_exchange_strong(expected, true,
                ...
                log::Info("First update tick with live lua_State — registering MemPatch + applying patches");
                mp::lua_bind::RegisterMemPatchTable(L);
                mp::patch::ApplyAll();
            }
        }
    }
...
uintptr_t FindUniqueSig(const pe::ModuleView& mod, const char* sig, const char* label) {
    auto pat = mp::patch::ParsePattern(sig);
    auto sections = pe::ExecutableSections(mod);
    std::vector<uintptr_t> all;
    for (const auto& sec : sections) {
        auto offs = mp::patch::FindAllInBuffer(sec.data, sec.size, pat);
