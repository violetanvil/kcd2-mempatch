...
DWORD WINAPI WorkerThread(LPVOID) {
    ...
    mp::log::Init(selfDir);
    mp::log::Info("");
    mp::log::Info("mempatch.asi loaded");
    char dirUtf8[512];
    WideCharToMultiByte(CP_UTF8, 0, selfDir.c_str(), -1, dirUtf8, sizeof(dirUtf8), nullptr, nullptr);
    mp::log::InfoF("module directory: %s", dirUtf8);

    // The ASI itself sits in plugins/, plugin subfolders are siblings of mempatch.asi.
    mp::config::LoadAllConfigs(selfDir);

    if (!mp::hooks::Install()) {
        mp::log::Error("hooks::Install failed — no patches will be applied");
        return 1;
    }
    return 0;
}
